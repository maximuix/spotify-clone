...
const upsertProductRecord = async (product: Stripe.Product) => {
    ...
    console.log(`Price inserted/updated: ${product.id}`);
...
const upsertPriceRecord = async (price: Stripe.Price) => {
    const priceData: Price = {
        ...
        trial_personal_days: price.recurring?.trial_period_days,
...
const manageSubscriptionStatusChange = async (
    ...
    } = await supabaseAdmin.from('customers').select('id').eq('stripe_customer_id', customerId);
